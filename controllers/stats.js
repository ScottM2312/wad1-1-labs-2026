...
const stats = {
  createView(request, response) {
    ...
    let maxSong = Math.max(...playlists.map(playlist => playlist.songs.length));
    let maxSongs = playlists.filter(playlist => playlist.songs.length === maxSong);
    let mostSongs = maxSongs.map(item => item.title);
